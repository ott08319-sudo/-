                    ...
                    if not data.get("skip") and not data.get("completed"):
                        ...
                        async with aiosqlite.connect(DB_PATH) as db:
                            async with db.execute(
                                "SELECT COUNT(*) FROM sponsor_tasks WHERE user_id = ? AND service = 'botohub'",
                                (user_id,)
                            ) as c:
                                count = await c.fetchone()
                                logging.info(f"Botohub сохранено заданий: {count[0] if count else 0}")
...
async def get_all_sponsors(user: types.User, force_refresh: bool = False):
    ...
    flyer = [s for s in flyer if not s.get("link", "").startswith("https://api.flyerpartners.com/")]
    for s in flyer:
...
def sponsors_keyboard(sponsors, page=1):
    ...
    for idx, sp in enumerate(page_sponsors, start + 1):
        if isinstance(sp, dict):
            ...
            link = sp  # Если это просто строка (Botohub)
