...
async def get_darkboost_sponsors(user_id: int, chat_id: int = None, username: str = "", first_name: str = "", last_name: str = "", language_code: str = "ru", is_premium: bool = False, max_sponsors: int = 10):
    if not DARKBOOST_API_KEY:
        await log_error(user_id, "darkboost", "no_api_key", "API ключ не установлен")
        return []
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, headers=headers, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    await log_error(user_id, "darkboost", "http_error", f"Status: {resp.status}")
    except Exception as e:
        await log_error(user_id, "darkboost", "api_error", str(e))
...
async def get_all_sponsors(user: types.User, force_refresh: bool = False):
    ...
    piarflow = [s for s in piarflow if is_telegram_link(s.get("link"))]
    for s in piarflow:
    ...
    flyer = [s for s in flyer if is_telegram_link(s.get("link"))]
    for s in flyer:
    ...
    tgrass = [s for s in tgrass if is_telegram_link(s.get("link"))]
    for s in tgrass:
    ...
    traffy = [s for s in traffy if is_telegram_link(s.get("target_link"))]
    for s in traffy:
    ...
    trafsly = [s for s in trafsly if is_telegram_link(s.get("link"))]
    for s in trafsly:
    ...
    darkboost = [s for s in darkboost if is_telegram_link(s.get("link"))]
    for s in darkboost:
    ...
    logging.info(f"Всего спонсоров (только Telegram): {len(all_sponsors)}")
...
def admin_keyboard():
    ...
    builder.row(
        types.InlineKeyboardButton(text="📊 Ошибки", callback_data="admin_errors"),
    ...
    for signature, assignment_id in flyer_tasks:
        if signature:
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                await db.commit()
            logging.info(f"✅ Задание Flyer без проверки засчитано: {assignment_id} для {user_id}")
...
async def admin_errors(callback: types.CallbackQuery):
    if callback.from_user.id != ADMIN_ID:
        await callback.answer("❌ Нет доступа!")
        return
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT user_id, service, error_type, message, created_at FROM error_log ORDER BY created_at DESC LIMIT 30"
        ) as c:
            errors = await c.fetchall()
    ...
    text = "📊 *Ошибки спонсоров (последние 30):*\n\n"
    for user_id, service, error_type, message, created_at in errors:
        date = datetime.fromtimestamp(created_at).strftime("%d.%m %H:%M")
        text += f"• [{date}] {service} → {error_type}\n"
        text += f"  📌 {message[:50]}...\n\n"
