...
async def check_all_subscriptions(user_id: int):
    ...
    for service, assignment_id, link, signature in tasks:
        if service == "flyer" and signature:
            ...
            if done:
                await db_execute("UPDATE sponsor_tasks SET status = 'subscribed' WHERE user_id = ? AND service = 'flyer' AND assignment_id = ?", (user_id, assignment_id))
            else:
                all_done = False
        elif service == "tgrass":
            done = await check_tgrass_subscription(user_id)
            if done:
                await db_execute("UPDATE sponsor_tasks SET status = 'subscribed' WHERE user_id = ? AND service = 'tgrass'", (user_id,))
            else:
                all_done = False
        elif service == "botohub":
            done = await check_botohub_tasks(user_id)
            if done:
                await db_execute("UPDATE sponsor_tasks SET status = 'subscribed' WHERE user_id = ? AND service = 'botohub'", (user_id,))
            else:
                all_done = False
    
    async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
        await db.execute("PRAGMA busy_timeout=5000;")
        async with db.execute("SELECT assignment_id, need_check FROM sponsor_tasks WHERE user_id = ? AND service = 'trafsly' AND status != 'subscribed'", (user_id,)) as c:
            trafsly_tasks = await c.fetchall()
    
    if trafsly_tasks:
        for assignment_id, need_check in trafsly_tasks:
            if need_check == 0:
                await db_execute("UPDATE sponsor_tasks SET status = 'subscribed' WHERE user_id = ? AND service = 'trafsly' AND assignment_id = ?", (user_id, assignment_id))
            else:
                try:
...
async def sponsors_page(callback: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
        await db.execute("PRAGMA busy_timeout=5000;")
        async with db.execute("SELECT payload FROM sponsor_cache WHERE cache_key = ?", (f"all_sponsors_{user_id}",)) as c:
            row = await c.fetchone()
...
async def check_subs(callback: types.CallbackQuery):
    ...
    if piarflow_tasks:
        ...
        for r in results:
            if r.get("status") in ["subscribed", "not_counted"]:
                await db_execute("UPDATE sponsor_tasks SET status = 'subscribed' WHERE user_id = ? AND service = 'piarflow' AND link = ?", (user_id, r.get("link")))
    ...
    if await check_tgrass_subscription(user_id):
        ...
        async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
            await db.execute("PRAGMA busy_timeout=5000;")
            async with db.execute("SELECT COUNT(*) FROM sponsor_tasks WHERE user_id = ? AND service = 'tgrass' AND status != 'subscribed'", (user_id,)) as c:
                count = await c.fetchone()
    ...
    if traffy_tasks:
        ...
        for r in results:
            if r.get("status") == "completed":
                await db_execute("UPDATE sponsor_tasks SET status = 'subscribed' WHERE user_id = ? AND service = 'traffy' AND assignment_id = ?", (user_id, r.get("assignment_id")))
    ...
    if all_done:
        ...
        async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
            await db.execute("PRAGMA busy_timeout=5000;")
            async with db.execute("SELECT link, service FROM sponsor_tasks WHERE user_id = ? AND status != 'subscribed'", (user_id,)) as c:
        ...
        if unsubscribed:
            ...
            # БЕЗ MARKDOWN! (ссылки ломают разметку)
            await bot.send_message(
                ADMIN_ID,
...
async def manual_allow(callback: types.CallbackQuery):
    ...
    # Помечаем ВСЕ неподтверждённые задания как выполненные
    await db_execute("UPDATE sponsor_tasks SET status = 'subscribed' WHERE user_id = ? AND status != 'subscribed'", (user_id,))
    await db_execute("UPDATE users SET is_activated = 1 WHERE user_id = ?", (user_id,))
    await db_execute("UPDATE manual_checks SET status = 'approved' WHERE user_id = ? AND status = 'pending'", (user_id,))
    ...
    await callback.message.edit_text(f"✅ Доступ разрешён для пользователя {user_id}")
...
async def manual_deny(callback: types.CallbackQuery):
    ...
    user_id = int(callback.data.split("_")[2])
    await db_execute("UPDATE manual_checks SET status = 'denied' WHERE user_id = ? AND status = 'pending'", (user_id,))
    
    try:
        await bot.send_message(user_id, "❌ *Доступ отклонён администратором.*\n\nПроверь подписки и попробуй снова.", parse_mode="Markdown")
    except:
        pass
    
    await callback.message.edit_text(f"❌ Доступ отклонён для пользователя {user_id}")
...
async def profile_cmd(message: types.Message):
    ...
    if user:
        async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
            await db.execute("PRAGMA busy_timeout=5000;")
            async with db.execute("SELECT COUNT(*) FROM users WHERE referrer_id = ? AND is_activated = 1", (user_id,)) as c:
...
async def sponsor_bonus(message: types.Message):
    ...
    if user.get("last_bonus") == today:
        await message.answer("⏳ *Ты уже забирал бонус сегодня!*\n🔄 Приходи завтра — будет новый бонус!", parse_mode="Markdown")
        return
    ...
    await db_execute("UPDATE users SET last_bonus = ? WHERE user_id = ?", (today, user_id))
...
async def admin_sponsor_status(callback: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH, timeout=30.0) as db:
        await db.execute("PRAGMA busy_timeout=5000;")
        async with db.execute("SELECT user_id, status, reason, updated_at FROM sponsor_status ORDER BY updated_at DESC LIMIT 20") as c:
...
async def admin_set_sponsor_reward_process(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if value <= 0:
            ...
            return
        await db_execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('sponsor_reward', ?)", (str(value),))
...
async def admin_set_max_sponsors_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Введи число от 1 до 30 (текущий: {current_max}):",
        parse_mode="Markdown"
    )
...
async def admin_set_max_sponsors_process(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if value < 1 or value > 30:
            ...
            return
        await db_execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('max_sponsors', ?)", (str(value),))
